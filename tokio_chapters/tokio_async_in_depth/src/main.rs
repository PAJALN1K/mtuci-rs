...
// Значение, возвращаемое my_async_fn(), является future.
// Future - это значение, которое реализует std::future::Future признак, предоставляемый 
//стандартной библиотекой. Это значения, которые содержат выполняемое асинхронное вычисление.
...
// В отличие от того, как фьючерсы реализуются в других языках, фьючерс Rust не представляет 
// вычисления, происходящие в фоновом режиме, скорее фьючерс Rust - это само вычисление. 
// Владелец future отвечает за ускорение вычислений путем опроса future. Это делается с 
// помощью вызова Future::poll.
...
// Давайте реализуем очень простое будущее. Это будущее будет:

// 1. Ждать до определенного момента времени.
// 2. Выводить некоторый текст в стандартный вывод.
// 3. Выдавать строку.
...
// Фьючерсы Rust - это конечные автоматы. Здесь MainFuture представлено как enum одно из возможных 
// состояний будущего. Будущее начинается с State0 состояния. При poll вызове future пытается 
// улучшить свое внутреннее состояние как можно больше. Если future может завершиться, Poll::Ready 
// возвращается содержащий выходные данные асинхронного вычисления.

// Если future не может завершиться, обычно из-за того, что ресурсы, которые он ожидает, не готовы, 
// то Poll::Pending возвращается. Получение Poll::Pending указывает вызывающему, что future 
// завершится позже, и вызывающий должен вызвать poll снова позже.
